CLASS zra_data_builder DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zra_data_builder IMPLEMENTATION.
...
*        output =
  ...
  METHOD fill_master_data.
    data : lt_bp type table of zra_bpa,
           lt_prod type table of zra_product.
   ...
   append value #(
    ...
    append value #(
     ...
     insert zra_bpa from table @lt_bp.
     insert zra_product from table @lt_prod.
  ENDMETHOD.
  ...
  METHOD fill_transaction_data.
    data : o_rand type REF TO cl_abap_random_int,
           ...
           lv_ord_id type zra_de_id,
           lt_so type table of zra_so_header,
           lt_so_i type table of zra_so_item.
    ...
    select * from zra_bpa into table @data(lt_bpa).
    select * from zra_product into table @data(lt_prod).
    ...
    insert zra_so_header from table @lt_so.
    insert zra_so_item from table @lt_so_i.

  ENDMETHOD.

  METHOD flush.
    delete from : zra_bpa, zra_product, zra_so_header, zra_so_item.
  ENDMETHOD.
